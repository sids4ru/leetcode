...
var minHeightShelves = function(books, shelf_width) {
	var DP = new Array(books.length);
	for(var i=0; i<books.length;i++){
		var cols = new Array(books.length);
		cols.fill(Infinity);
		DP[i] = cols;
	}
	return calcHeight(1,books[0][0],books[0][1],shelf_width,books,DP,0);
};
function calcHeight(pos,width,height,shelf_width,books,DP,level){
	...
	if(DP[pos][width]<Infinity)
		return DP[pos][width];
	
	var book = books[pos];
	var height1 = calcHeight(pos+1,width+book[0],Math.max(height,book[1]),shelf_width,books,DP,level);

	var height2 = calcHeight(pos+1,book[0], book[1],shelf_width,books,DP,level+1);
	var minHeight = Math.min(height1,height2+height);
	DP[pos][width] = minHeight;
	return minHeight;
}
